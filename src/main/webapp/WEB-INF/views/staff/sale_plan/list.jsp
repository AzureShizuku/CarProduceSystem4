...
<title>销售计划</title>
<meta name="keywords" content="销售计划">
<meta name="description" content="销售计划">
			...
			<div class="ibox float-e-margins">
				...
				<div class="ibox-content">
					<div class="row">
						<div class="col-lg-2">
							<label>计划名</label>
							<input type="text" class="form-control" value="" placeholder="请输入计划名" id="planName" >
						</div>
						<div class="col-lg-1">
					...
					<div class="row"  style="margin-top:5px;padding-left: 15px;">
						<a href="${pageContext.request.contextPath}/staff/sale_plan/add" class="btn btn-primary">新增</a>
					</div>
					...
					<div class="table-responsive">
						<table class="table  table-striped table-bordered  table-hover">
							...
							<tbody  id="sale_plan_body">
							</tbody>
						</table>
						<div id="sale_plan_bar" class="pagination" style="margin-top:20px;"></div>
					</div>
				</div>
			</div>
		</div>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=1250"></script>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/staff/sale_plan/queryList",
		data : {
			"page":page,
			"planName":planName
		},
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				html += '<tr>' ;
				var planNameVal=setNullToEmpty(rows[i].salePlan.planName);
				html+='<td>'+ planNameVal+'</td>';
				var ksDateVal=setNullToEmpty(rows[i].salePlan.ksDate);
				html+='<td>'+ ksDateVal+'</td>';
				var jsDateVal=setNullToEmpty(rows[i].salePlan.jsDate);
				html+='<td>'+ jsDateVal+'</td>';
				var createDateVal=setNullToEmpty(rows[i].salePlan.createDate);
				html+='<td>'+ createDateVal+'</td>';
				html+='<td>';
				html+='<a href="${pageContext.request.contextPath}/staff/sale_plan/detail2?id='+rows[i].salePlan.id+'" class="btn btn-primary btnStyle">查看详情</a>';
				html+='<a href="${pageContext.request.contextPath}/staff/sale_plan/update?id='+rows[i].salePlan.id+'" class="btn btn-success btnStyle">修改</a>';
				html+='<a href="javascript:void(0)" onclick="del1(this)" data-id="'+rows[i].salePlan.id+'" class="btn btn-danger btnStyle">删除</a>';
				html+='</td>';
				html+='</tr>';
			}
			if (rows.length == 0) {
...
function ajaxCount() {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/staff/sale_plan/queryCount",
		async: false,
		data : {
			"planName":planName
		},
...
function del1(e){
	if(window.confirm('你确定要删除该销售计划吗')){
		var id = $(e).attr("data-id");
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/staff/sale_plan/del1",
			data : {
				"id":id
			},
			success : function(result) {
