...
<title>修改-物料入库单</title>
<meta name="keywords" content="修改-物料入库单">
<meta name="description" content="修改-物料入库单">
			...
			<div class="ibox float-e-margins">
				...
				<div class="ibox-content">
					<div class="row"   style="width:80%;margin-left:10%">
						<div class="col-lg-6">
							<div class="form-group" id="materialIdParentContent">
								<label>物料ID<span style="color:red">(*必填)</span></label>
								<select  id="materialId"   >
									<c:forEach items="${materialInfoList}" var="item">
										<c:if test="${item.id==data.materialId}">
							...
							<div class="form-group" id="numParentContent">
								<label>入库数量<span style="color:red">(*必填)</span></label>
								<input type="text" value="${data.num}" placeholder="请输入入库数量(请输入数字)" id="num" class="form-control">
							</div>
							<div class="form-group" id="inRemarkParentContent">
								<label>入库备注<span style="color:red">(*必填)</span></label>
								<input type="text" value="${data.inRemark}" placeholder="请输入入库备注" id="inRemark" class="form-control">
							</div>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=0995"></script>
...
function submitData(){
	...
	$.ajax({
		type: 'post',
		url: '${pageContext.request.contextPath}/staff/minlog/update_submit',
		data:{
			"id":'${data.id}',
		...
		success: function(result) {
			if(result.code == 0){
				...
				alert(result.msg);
				window.location.href="${pageContext.request.contextPath}/staff/minlog";
			}
		}
	});
}
