			...
			<div class="ibox float-e-margins">
				...
				<div class="ibox-content">
					...
					<div class="row"  style="margin-top:5px;padding-left: 15px;">
						<a href="${pageContext.request.contextPath}/staff/minlog/add" class="btn btn-primary">新增</a>
					</div>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=7753"></script>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/staff/minlog/queryList",
		data : {
			"page":page,
		...
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				...
				html+='<td>';
				if(rows[i].minlog.isComfirm=='1'){
					html+='<a href="${pageContext.request.contextPath}/staff/minlog/update?id='+rows[i].minlog.id+'" class="btn btn-success btnStyle">修改</a>';
				}
				if(rows[i].minlog.isComfirm=='1'){
					html+='<a href="javascript:void(0)" onclick="del1(this)" data-id="'+rows[i].minlog.id+'" class="btn btn-danger btnStyle">删除</a>';
				}
				if(rows[i].minlog.isComfirm=='1'){
					html+='<a href="javascript:void(0)" onclick="qr(this)" data-id="'+rows[i].minlog.id+'" act-type="1" class="btn btn-primary btnStyle">确认</a>';
				}
				html+='</td>';
				html+='</tr>';
			}
...
function ajaxCount() {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/staff/minlog/queryCount",
		async: false,
		data : {
			"materialId":materialId,
			"createDate1":createDate1,
...
function qr(e){
	var r=confirm("确认确认该物料入库单？");
	if(r==true){
		var id = $(e).attr("data-id");
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/staff/minlog/qr",
			data : {
				"id":id
			},
			success : function(result) {
...
function del1(e){
	if(window.confirm('你确定要删除该物料入库单吗')){
		...
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/staff/minlog/del1",
			data : {
				"id":id
			},
			success : function(result) {
