				...
				<div class="ibox-content">
					...
					<div class="row"  style="margin-top:5px;padding-left: 15px;">
						<c:if test="${user.staffType==2}">
							<a href="${pageContext.request.contextPath}/staff/pro_request/add" class="btn btn-primary">新增</a>
						</c:if>
					</div>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=4698"></script>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/staff/pro_request/queryList",
		data : {
			"page":page,
			"proNo":proNo,
			"proName":proName,
		...
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				...
				html+='<td>';
				if('${user.staffType}'=='2'&&rows[i].proRequest.requestStatus=='1'){
					html+='<a href="${pageContext.request.contextPath}/staff/pro_request/update?id='+rows[i].proRequest.id+'" class="btn btn-success btnStyle">修改</a>';
				}
				if('${user.staffType}'=='2'&&rows[i].proRequest.requestStatus=='1'){
					html+='<a href="javascript:void(0)" onclick="del1(this)" data-id="'+rows[i].proRequest.id+'" class="btn btn-danger btnStyle">删除</a>';
				}
				if('${user.staffType}'=='5'&&rows[i].proRequest.requestStatus=='1'){
					html+='<a href="${pageContext.request.contextPath}/staff/pro_request/jssq?id='+rows[i].proRequest.id+'" class="btn btn-primary btnStyle">接受申请</a>';
				}
				if('${user.staffType}'=='5'&&rows[i].proRequest.requestStatus=='1'){
					html+='<a href="${pageContext.request.contextPath}/staff/pro_request/jjsq?id='+rows[i].proRequest.id+'" class="btn btn-danger btnStyle">拒绝申请</a>';
				}
				html+='</td>';
				html+='</tr>';
			}
...
function ajaxCount() {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/staff/pro_request/queryCount",
		async: false,
		data : {
			"proNo":proNo,
			"proName":proName,
...
function del1(e){
	if(window.confirm('你确定要删除该生产请求吗')){
		...
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/staff/pro_request/del1",
			data : {
				"id":id
			},
			success : function(result) {
