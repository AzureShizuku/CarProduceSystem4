				...
				<div class="ibox-content">
					...
					<div class="row"  style="margin-top:5px;padding-left: 15px;">
						<c:if test="${user.staffType==2}">
							<a href="${pageContext.request.contextPath}/staff/poutlog/add" class="btn btn-primary">新增</a>
						</c:if>
					</div>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=7179"></script>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/staff/poutlog/queryList",
		data : {
			"page":page,
			"proNo":proNo,
			"proName":proName,
		...
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				...
				html+='<td>';
				if('${user.staffType}'=='2'&&rows[i].poutlog.status=='1'){
					html+='<a href="${pageContext.request.contextPath}/staff/poutlog/update?id='+rows[i].poutlog.id+'" class="btn btn-success btnStyle">修改</a>';
				}
				if('${user.staffType}'=='2'&&rows[i].poutlog.status=='1'){
					html+='<a href="javascript:void(0)" onclick="del1(this)" data-id="'+rows[i].poutlog.id+'" class="btn btn-danger btnStyle">删除</a>';
				}
				if('${user.staffType}'=='2'&&rows[i].poutlog.status=='1'){
					html+='<a href="javascript:void(0)" onclick="qr(this)" data-id="'+rows[i].poutlog.id+'" act-type="1" class="btn btn-primary btnStyle">确认</a>';
				}
				html+='</td>';
				html+='</tr>';
			}
...
function ajaxCount() {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/staff/poutlog/queryCount",
		async: false,
		data : {
			"proNo":proNo,
			"proName":proName,
...
function qr(e){
	var r=confirm("确认确认该出库单？");
	if(r==true){
		var id = $(e).attr("data-id");
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/staff/poutlog/qr",
			data : {
				"id":id
			},
			success : function(result) {
...
function del1(e){
	if(window.confirm('你确定要删除该出库单吗')){
		...
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/staff/poutlog/del1",
			data : {
				"id":id
			},
			success : function(result) {
