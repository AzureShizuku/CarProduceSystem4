...
<title>修改-物料台账</title>
<meta name="keywords" content="修改-物料台账">
<meta name="description" content="修改-物料台账">
			...
			<div class="ibox float-e-margins">
				...
				<div class="ibox-content">
					<div class="row"   style="width:80%;margin-left:10%">
						<div class="col-lg-6">
							<div class="form-group" id="mnoParentContent">
								<label>物料编号<span style="color:red">(*必填)</span></label>
								<input type="text" value="${data.mno}" placeholder="请输入物料编号" id="mno" class="form-control">
							</div>
							<div class="form-group" id="mnameParentContent">
								<label>物料名<span style="color:red">(*必填)</span></label>
								<input type="text" value="${data.mname}" placeholder="请输入物料名" id="mname" class="form-control">
							</div>
							<div class="form-group" id="mintroParentContent">
								<label>物料介绍<span style="color:red">(*必填)</span></label>
								<input type="text" value="${data.mintro}" placeholder="请输入物料介绍" id="mintro" class="form-control">
							</div>
							<div class="form-group" id="stockNumParentContent">
								<label>库存量<span style="color:red">(*必填)</span></label>
								<input type="text" value="${data.stockNum}" placeholder="请输入库存量(请输入数字)" id="stockNum" class="form-control">
							</div>
							...
							<div class="form-group" id="providerIdParentContent">
								<label>供应商<span style="color:red">(*必填)</span></label>
								<select  id="providerId"   >
									<c:forEach items="${providerList}" var="item">
										<c:if test="${item.id==data.providerId}">
							...
							<div class="form-group" id="jsbzParentContent">
								...
								<input type="text" value="${data.jsbz}" placeholder="请输入技术标准" id="jsbz" class="form-control">
							</div>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=7606"></script>
...
function submitData(){
	...
	$.ajax({
		type: 'post',
		url: '${pageContext.request.contextPath}/staff/material_info/update1_submit',
		data:{
			"id":'${data.id}',
			"mno":mno,
			"mname":mname,
		...
		success: function(result) {
			if(result.code == 0){
				...
				alert(result.msg);
				window.location.href="${pageContext.request.contextPath}/staff/material_info";
			}
		}
	});
}
