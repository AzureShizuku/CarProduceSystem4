...
<title>物料申购单</title>
<meta name="keywords" content="物料申购单">
<meta name="description" content="物料申购单">
			...
			<div class="ibox float-e-margins">
				...
				<div class="ibox-content">
					<div class="row">
						<div class="col-lg-2">
							<label>物料编号</label>
							<input type="text" class="form-control" value="" placeholder="请输入物料编号" id="no" >
						</div>
						<div class="col-lg-2">
							<label>物料名</label>
					...
					<div class="row"  style="margin-top:5px;padding-left: 15px;">
						<c:if test="${user.staffType==2}">
							<a href="${pageContext.request.contextPath}/staff/mpurchase_app/add" class="btn btn-primary">新增</a>
						</c:if>
					</div>
					...
					<div class="table-responsive">
						<table class="table  table-striped table-bordered  table-hover">
							...
							<tbody  id="mpurchase_app_body">
							</tbody>
						</table>
						<div id="mpurchase_app_bar" class="pagination" style="margin-top:20px;"></div>
					</div>
				</div>
			</div>
		</div>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=4867"></script>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/staff/mpurchase_app/queryList",
		data : {
			"page":page,
			"no":no,
			"name":name,
		...
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				html += '<tr>' ;
				var noVal=setNullToEmpty(rows[i].mpurchaseApp.no);
				html+='<td>'+ noVal+'</td>';
				var nameVal=setNullToEmpty(rows[i].mpurchaseApp.name);
				html+='<td>'+ nameVal+'</td>';
				var ggVal=setNullToEmpty(rows[i].mpurchaseApp.gg);
				html+='<td>'+ ggVal+'</td>';
				var mtypeVal=setNullToEmpty(rows[i].mpurchaseApp.mtype);
				html+='<td>'+ mtypeVal+'</td>';
				var pintroVal=setNullToEmpty(rows[i].mpurchaseApp.pintro);
				html+='<td>'+ pintroVal+'</td>';
				var mstandardVal=setNullToEmpty(rows[i].mpurchaseApp.mstandard);
				html+='<td>'+ mstandardVal+'</td>';
				var numVal=setNullToEmpty(rows[i].mpurchaseApp.num);
				html+='<td>'+ numVal+'</td>';
				var priceVal=setNullToEmpty(rows[i].mpurchaseApp.price);
				html+='<td>￥'+priceVal+'</td>';
				var planPriceVal=setNullToEmpty(rows[i].mpurchaseApp.planPrice);
				html+='<td>￥'+planPriceVal+'</td>';
				var totalAmountVal=setNullToEmpty(rows[i].mpurchaseApp.totalAmount);
				html+='<td>'+ totalAmountVal+'</td>';
				var dhDateVal=setNullToEmpty(rows[i].mpurchaseApp.dhDate);
				html+='<td>'+ dhDateVal+'</td>';
				var checkStatusVal=setNullToEmpty(rows[i].checkStatusStr);
				html+='<td>'+checkStatusVal+'</td>';
				var shbzVal=setNullToEmpty(rows[i].mpurchaseApp.shbz);
				html+='<td>'+ shbzVal+'</td>';
				html+='<td>';
				if('${user.staffType}'=='2'&&rows[i].mpurchaseApp.checkStatus=='1'){
					html+='<a href="${pageContext.request.contextPath}/staff/mpurchase_app/update?id='+rows[i].mpurchaseApp.id+'" class="btn btn-success btnStyle">修改</a>';
				}
				if('${user.staffType}'=='2'&&rows[i].mpurchaseApp.checkStatus=='1'){
					html+='<a href="javascript:void(0)" onclick="del1(this)" data-id="'+rows[i].mpurchaseApp.id+'" class="btn btn-danger btnStyle">删除</a>';
				}
				if('${user.staffType}'=='6'&&rows[i].mpurchaseApp.checkStatus=='1'){
					html+='<a href="${pageContext.request.contextPath}/staff/mpurchase_app/jssq?id='+rows[i].mpurchaseApp.id+'" class="btn btn-primary btnStyle">接受申请</a>';
				}
				if('${user.staffType}'=='6'&&rows[i].mpurchaseApp.checkStatus=='1'){
					html+='<a href="${pageContext.request.contextPath}/staff/mpurchase_app/jjsq?id='+rows[i].mpurchaseApp.id+'" class="btn btn-danger btnStyle">拒绝申请</a>';
				}
				html+='</td>';
				html+='</tr>';
			}
			...
			$("#mpurchase_app_body").html(html);
			new myPagination({
				id: 'mpurchase_app_bar',
				curPage:page, //初始页码
...
function ajaxCount() {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/staff/mpurchase_app/queryCount",
		async: false,
		data : {
			"no":no,
			"name":name,
...
function del1(e){
	if(window.confirm('你确定要删除该物料申购单吗')){
		var id = $(e).attr("data-id");
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/staff/mpurchase_app/del1",
			data : {
				"id":id
			},
			success : function(result) {
