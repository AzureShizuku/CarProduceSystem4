...
<title>库存台账</title>
<meta name="keywords" content="库存台账">
<meta name="description" content="库存台账">
				...
				<div class="ibox-content">
					...
					<div class="row"  style="margin-top:5px;padding-left: 15px;">
						<c:if test="${user.staffType==2}">
							<a href="${pageContext.request.contextPath}/staff/stock_msg/add1" class="btn btn-primary">新增</a>
						</c:if>
					</div>
					...
					<div class="table-responsive">
						<table class="table  table-striped table-bordered  table-hover">
							...
							<tbody  id="stock_msg_body">
							</tbody>
						</table>
						<div id="stock_msg_bar" class="pagination" style="margin-top:20px;"></div>
					</div>
				</div>
			</div>
		</div>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=1125"></script>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/staff/stock_msg/queryList",
		data : {
			"page":page,
			"proNo":proNo,
			"proName":proName
		...
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				html += '<tr>' ;
				var proNoVal=setNullToEmpty(rows[i].stockMsg.proNo);
				html+='<td>'+ proNoVal+'</td>';
				var proNameVal=setNullToEmpty(rows[i].stockMsg.proName);
				html+='<td>'+ proNameVal+'</td>';
				var ggVal=setNullToEmpty(rows[i].stockMsg.gg);
				html+='<td>'+ ggVal+'</td>';
				var dwVal=setNullToEmpty(rows[i].stockMsg.dw);
				html+='<td>'+ dwVal+'</td>';
				var priceVal=setNullToEmpty(rows[i].stockMsg.price);
				html+='<td>￥'+priceVal+'</td>';
				var numVal=setNullToEmpty(rows[i].stockMsg.num);
				html+='<td>'+ numVal+'</td>';
				html+='<td>';
				if('${user.staffType}'=='2'){
					html+='<a href="${pageContext.request.contextPath}/staff/stock_msg/update1?id='+rows[i].stockMsg.id+'" class="btn btn-success btnStyle">修改</a>';
				}
				if('${user.staffType}'=='2'){
					html+='<a href="javascript:void(0)" onclick="del1(this)" data-id="'+rows[i].stockMsg.id+'" class="btn btn-danger btnStyle">删除</a>';
				}
				html+='</td>';
				html+='</tr>';
			}
...
function ajaxCount() {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/staff/stock_msg/queryCount",
		async: false,
		data : {
			"proNo":proNo,
			"proName":proName
...
function del1(e){
	if(window.confirm('你确定要删除该库存台账吗')){
		var id = $(e).attr("data-id");
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/staff/stock_msg/del1",
			data : {
				"id":id
			},
			success : function(result) {
