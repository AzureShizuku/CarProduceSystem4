...
<title>修改-销售订单</title>
<meta name="keywords" content="修改-销售订单">
<meta name="description" content="修改-销售订单">
					...
					<div class="row"   style="width:80%;margin-left:10%">
						<div class="col-lg-6">
							...
							<div class="form-group" id="numParentContent">
								...
								<input type="text" value="${data.num}" placeholder="请输入销售数量(请输入数字)" id="num" class="form-control">
							</div>
							<div class="form-group" id="amountParentContent">
								<label>销售金额<span style="color:red">(*必填)</span></label>
								<input type="text" value="${data.amount}" placeholder="请输入销售金额(请输入数字)" id="amount" class="form-control">
							</div>
							<div class="form-group" id="htFileParentContent">
								<label>销售合同<span style="color:red">(*必填)</span></label>
								<div id="htFile" class="dropzone" data-id="htFile" max-num="1" file-type="4" max-size="1000" init-val="${data.htFile}"></div>
							</div>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=3915"></script>
...
function submitData(){
	...
	$.ajax({
		type: 'post',
		url: '${pageContext.request.contextPath}/staff/sale_log/update_submit',
		data:{
			"id":'${data.id}',
			"stockId":stockId,
			"num":num,
