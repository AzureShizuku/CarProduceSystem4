...
<title>物料采购计划</title>
<meta name="keywords" content="物料采购计划">
<meta name="description" content="物料采购计划">
				...
				<div class="ibox-content">
					...
					<div class="row"  style="margin-top:5px;padding-left: 15px;">
						<c:if test="${user.staffType==6}">
							<a href="${pageContext.request.contextPath}/staff/mpurchase_plan/add" class="btn btn-primary">新增</a>
						</c:if>
					</div>
					...
					<div class="table-responsive">
						<table class="table  table-striped table-bordered  table-hover">
							...
							<tbody  id="mpurchase_plan_body">
							</tbody>
						</table>
						<div id="mpurchase_plan_bar" class="pagination" style="margin-top:20px;"></div>
					</div>
				</div>
			</div>
		</div>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=1199"></script>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/staff/mpurchase_plan/queryList",
		data : {
			"page":page,
			"title":title
		},
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				html += '<tr>' ;
				var titleVal=setNullToEmpty(rows[i].mpurchasePlan.title);
				html+='<td>'+ titleVal+'</td>';
				var pintroVal=setNullToEmpty(rows[i].mpurchasePlan.pintro);
				html+='<td>'+ pintroVal+'</td>';
				var pdetailStr = rows[i].mpurchasePlan.pdetail;
				if(pdetailStr!=null){
					var pdetailSplit = pdetailStr.split(";");
					var tmp = '';
					for(var j=0;j<pdetailSplit.length;j++){
						if(pdetailSplit[j]!=''){
							tmp+='<div ><a  href="javascript:void(0)" file-url="'+pdetailSplit[j]+'" onclick="downLoadFile(this)">点击下载采购详情</a></div>';
						}
					}
					html+='<td>'+tmp+'</td>';
					}else{
				...
				var totalAmountVal=setNullToEmpty(rows[i].mpurchasePlan.totalAmount);
				html+='<td>'+ totalAmountVal+'</td>';
				var jfDateVal=setNullToEmpty(rows[i].mpurchasePlan.jfDate);
				html+='<td>'+ jfDateVal+'</td>';
				html+='<td>';
				if('${user.staffType}'=='6'){
					html+='<a href="${pageContext.request.contextPath}/staff/mpurchase_plan/update?id='+rows[i].mpurchasePlan.id+'" class="btn btn-success btnStyle">修改</a>';
				}
				html+='</td>';
				html+='</tr>';
			}
			...
			$("#mpurchase_plan_body").html(html);
			new myPagination({
				id: 'mpurchase_plan_bar',
				curPage:page, //初始页码
...
function ajaxCount() {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/staff/mpurchase_plan/queryCount",
		async: false,
		data : {
			"title":title
		},
