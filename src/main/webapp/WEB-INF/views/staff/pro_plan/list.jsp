...
<title>生产计划</title>
<meta name="keywords" content="生产计划">
<meta name="description" content="生产计划">
			...
			<div class="ibox float-e-margins">
				...
				<div class="ibox-content">
					<div class="row">
						<div class="col-lg-2">
							<label>计划名</label>
							<input type="text" class="form-control" value="" placeholder="请输入计划名" id="pname" >
						</div>
						<div class="col-lg-1">
					...
					<div class="row"  style="margin-top:5px;padding-left: 15px;">
						<a href="${pageContext.request.contextPath}/staff/pro_plan/add" class="btn btn-primary">新增</a>
					</div>
					...
					<div class="table-responsive">
						<table class="table  table-striped table-bordered  table-hover">
							...
							<tbody  id="pro_plan_body">
							</tbody>
						</table>
						<div id="pro_plan_bar" class="pagination" style="margin-top:20px;"></div>
					</div>
				</div>
			</div>
		</div>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=7364"></script>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/staff/pro_plan/queryList",
		data : {
			"page":page,
			"pname":pname
		},
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				html += '<tr>' ;
				var pnameVal=setNullToEmpty(rows[i].proPlan.pname);
				html+='<td>'+ pnameVal+'</td>';
				var pintroVal=setNullToEmpty(rows[i].proPlan.pintro);
				html+='<td>'+ pintroVal+'</td>';
				var pdetailStr = rows[i].proPlan.pdetail;
				if(pdetailStr!=null){
					var pdetailSplit = pdetailStr.split(";");
					var tmp = '';
					for(var j=0;j<pdetailSplit.length;j++){
						if(pdetailSplit[j]!=''){
							tmp+='<div ><a  href="javascript:void(0)" file-url="'+pdetailSplit[j]+'" onclick="downLoadFile(this)">点击下载生产详情单</a></div>';
						}
					}
					html+='<td>'+tmp+'</td>';
					}else{
				...
				var ksDateVal=setNullToEmpty(rows[i].proPlan.ksDate);
				html+='<td>'+ ksDateVal+'</td>';
				var jsDateVal=setNullToEmpty(rows[i].proPlan.jsDate);
				html+='<td>'+ jsDateVal+'</td>';
				var createDateVal=setNullToEmpty(rows[i].proPlan.createDate);
				html+='<td>'+ createDateVal+'</td>';
				html+='<td>';
				html+='<a href="${pageContext.request.contextPath}/staff/pro_plan/update?id='+rows[i].proPlan.id+'" class="btn btn-success btnStyle">修改</a>';
				html+='<a href="javascript:void(0)" onclick="del1(this)" data-id="'+rows[i].proPlan.id+'" class="btn btn-danger btnStyle">删除</a>';
				html+='</td>';
				html+='</tr>';
			}
			if (rows.length == 0) {
...
function ajaxCount() {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/staff/pro_plan/queryCount",
		async: false,
		data : {
			"pname":pname
		},
...
function del1(e){
	if(window.confirm('你确定要删除该生产计划吗')){
		var id = $(e).attr("data-id");
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/staff/pro_plan/del1",
			data : {
				"id":id
			},
			success : function(result) {
