...
<title>新增-生产计划</title>
<meta name="keywords" content="新增-生产计划">
<meta name="description" content="新增-生产计划">
			...
			<div class="ibox float-e-margins">
				...
				<div class="ibox-content">
					<div class="row"   style="width:80%;margin-left:10%">
						<div class="col-lg-6">
							<div class="form-group" id="pnameParentContent">
								<label>计划名<span style="color:red">(*必填)</span></label>
								<input type="text" value="" placeholder="请输入计划名" id="pname" class="form-control">
							</div>
							<div class="form-group" id="pintroParentContent">
								<label>生产介绍<span style="color:red">(*必填)</span></label>
								<textarea class="form-control"  placeholder="请输入生产介绍"  id="pintro"  rows="4" ></textarea>
							</div>
							<div class="form-group" id="pdetailParentContent">
								<label>生产详情单<span style="color:red">(*必填)</span></label>
								<div id="pdetail" class="dropzone" data-id="pdetail" max-num="1" file-type="4" max-size="1000" init-val=""></div>
							</div>
							<div class="form-group" id="ksDateParentContent">
								<label>开始日期<span style="color:red">(*必填)</span></label>
								<input type="text" class="form-control Wdate"  readonly="readonly"  value=""  onFocus="WdatePicker({startDate: '%y-%M-%d' ,dateFmt:'yyyy-MM-dd',alwaysUseStartDate: true })"  placeholder="请输入开始日期" id="ksDate">
							</div>
							<div class="form-group" id="jsDateParentContent">
								<label>结束日期<span style="color:red">(*必填)</span></label>
								<input type="text" class="form-control Wdate"  readonly="readonly"  value=""  onFocus="WdatePicker({startDate: '%y-%M-%d' ,dateFmt:'yyyy-MM-dd',alwaysUseStartDate: true })"  placeholder="请输入结束日期" id="jsDate">
							</div>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=1732"></script>
...
function submitData(){
	...
	var pdetail=getFileVal2('pdetail')
	var ksDate= $('#ksDate').val();
	var jsDate= $('#jsDate').val();
	$.ajax({
		type: 'post',
		url: '${pageContext.request.contextPath}/staff/pro_plan/add_submit',
		data:{
			"pname":pname,
			"pintro":pintro,
			"pdetail":pdetail,
		...
		success: function(result) {
			if(result.code == 0){
				...
				alert(result.msg);
				window.location.href="${pageContext.request.contextPath}/staff/pro_plan";
			}
		}
	});
}
