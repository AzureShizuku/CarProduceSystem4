...
<title>修改-生产计划</title>
<meta name="keywords" content="修改-生产计划">
<meta name="description" content="修改-生产计划">
					...
					<div class="row"   style="width:80%;margin-left:10%">
						<div class="col-lg-6">
							<div class="form-group" id="pnameParentContent">
								...
								<input type="text" value="${data.pname}" placeholder="请输入计划名" id="pname" class="form-control">
							</div>
							<div class="form-group" id="pintroParentContent">
								<label>生产介绍<span style="color:red">(*必填)</span></label>
								<textarea class="form-control"  placeholder="请输入生产介绍"  id="pintro"  rows="4" >${data.pintro}</textarea>
							</div>
							<div class="form-group" id="pdetailParentContent">
								<label>生产详情单<span style="color:red">(*必填)</span></label>
								<div id="pdetail" class="dropzone" data-id="pdetail" max-num="1" file-type="4" max-size="1000" init-val="${data.pdetail}"></div>
							</div>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=8657"></script>
...
function submitData(){
	...
	$.ajax({
		type: 'post',
		url: '${pageContext.request.contextPath}/staff/pro_plan/update_submit',
		data:{
			"id":'${data.id}',
			"pname":pname,
			"pintro":pintro,
