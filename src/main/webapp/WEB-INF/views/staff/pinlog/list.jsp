			...
			<div class="ibox float-e-margins">
				...
				<div class="ibox-content">
					...
					<div class="row"  style="margin-top:5px;padding-left: 15px;">
						<a href="${pageContext.request.contextPath}/staff/pinlog/add" class="btn btn-primary">新增</a>
					</div>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=5914"></script>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/staff/pinlog/queryList",
		data : {
			"page":page,
			"proNo":proNo,
			"proName":proName,
		...
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				...
				html+='<td>';
				if(rows[i].pinlog.status=='1'){
					html+='<a href="${pageContext.request.contextPath}/staff/pinlog/update?id='+rows[i].pinlog.id+'" class="btn btn-success btnStyle">修改</a>';
				}
				if(rows[i].pinlog.status=='1'){
					html+='<a href="javascript:void(0)" onclick="del(this)" data-id="'+rows[i].pinlog.id+'" class="btn btn-danger btnStyle">删除</a>';
				}
				if(rows[i].pinlog.status=='1'){
					html+='<a href="javascript:void(0)" onclick="qr(this)" data-id="'+rows[i].pinlog.id+'" act-type="1" class="btn btn-primary btnStyle">确认</a>';
				}
				html+='</td>';
				html+='</tr>';
			}
...
function ajaxCount() {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/staff/pinlog/queryCount",
		async: false,
		data : {
			"proNo":proNo,
			"proName":proName,
...
function qr(e){
	var r=confirm("确认确认该入库记录？");
	if(r==true){
		var id = $(e).attr("data-id");
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/staff/pinlog/qr",
			data : {
				"id":id
			},
			success : function(result) {
...
function del(e){
	if(window.confirm('你确定要删除该入库记录吗')){
		var id = $(e).attr("data-id");
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/staff/pinlog/del",
			data : {
				"id":id
			},
			success : function(result) {
