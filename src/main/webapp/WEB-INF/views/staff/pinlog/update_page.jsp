...
<title>修改-入库记录</title>
<meta name="keywords" content="修改-入库记录">
<meta name="description" content="修改-入库记录">
					...
					<div class="row"   style="width:80%;margin-left:10%">
						<div class="col-lg-6">
							<div class="form-group" id="stockIdParentContent">
								<label>台账ID<span style="color:red">(*必填)</span></label>
								<select  id="stockId"   >
							...
							<div class="form-group" id="numParentContent">
								<label>商品数<span style="color:red">(*必填)</span></label>
								<input type="text" value="${data.num}" placeholder="请输入商品数(请输入数字)" id="num" class="form-control">
							</div>
							<div class="form-group" id="remarkParentContent">
								<label>入库备注<span style="color:red">(*必填)</span></label>
								<input type="text" value="${data.remark}" placeholder="请输入入库备注" id="remark" class="form-control">
							</div>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=9433"></script>
...
function submitData(){
	...
	$.ajax({
		type: 'post',
		url: '${pageContext.request.contextPath}/staff/pinlog/update_submit',
		data:{
			"id":'${data.id}',
			"stockId":stockId,
			"num":num,
		...
		success: function(result) {
			if(result.code == 0){
				...
				alert(result.msg);
				window.location.href="${pageContext.request.contextPath}/staff/pinlog";
			}
		}
	});
}
