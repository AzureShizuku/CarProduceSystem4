...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=1780"></script>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/provider_app/queryList",
		data : {
			"page":page,
			"name":name,
			"evalType":evalType
		...
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				...
				html+='<td>';
				html+='<a href="javascript:void(0)" onclick="del1(this)" data-id="'+rows[i].providerApp.id+'" class="btn btn-danger btnStyle">删除</a>';
				html+='</td>';
				html+='</tr>';
			}
			if (rows.length == 0) {
...
function ajaxCount() {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/provider_app/queryCount",
		async: false,
		data : {
			"name":name,
			"evalType":evalType
...
function del1(e){
	if(window.confirm('你确定要删除该供应商申请吗')){
		...
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/admin/provider_app/del1",
			data : {
				"id":id
			},
			success : function(result) {
