...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=7956"></script>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/sale_plan/queryList",
		data : {
			"page":page,
			"planName":planName
		},
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				...
				html+='<td>';
				html+='<a href="${pageContext.request.contextPath}/admin/sale_plan/detail1?id='+rows[i].salePlan.id+'" class="btn btn-primary btnStyle">查看详情</a>';
				html+='<a href="javascript:void(0)" onclick="del(this)" data-id="'+rows[i].salePlan.id+'" class="btn btn-danger btnStyle">删除</a>';
				html+='</td>';
				html+='</tr>';
			}
			if (rows.length == 0) {
...
function ajaxCount() {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/sale_plan/queryCount",
		async: false,
		data : {
			"planName":planName
		},
...
function del(e){
	if(window.confirm('你确定要删除该销售计划吗')){
		...
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/admin/sale_plan/del",
			data : {
				"id":id
			},
			success : function(result) {
