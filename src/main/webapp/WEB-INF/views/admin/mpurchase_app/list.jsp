...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=2191"></script>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/mpurchase_app/queryList",
		data : {
			"page":page,
			"no":no,
			"name":name,
		...
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				...
				html+='<td>'+ shbzVal+'</td>';
				html+='<td>';
				html+='<a href="javascript:void(0)" onclick="del(this)" data-id="'+rows[i].mpurchaseApp.id+'" class="btn btn-danger btnStyle">删除</a>';
				html+='</td>';
				html+='</tr>';
			}
			if (rows.length == 0) {
...
function ajaxCount() {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/mpurchase_app/queryCount",
		async: false,
		data : {
			"no":no,
			"name":name,
...
function del(e){
	if(window.confirm('你确定要删除该物料申购单吗')){
		...
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/admin/mpurchase_app/del",
			data : {
				"id":id
			},
			success : function(result) {
