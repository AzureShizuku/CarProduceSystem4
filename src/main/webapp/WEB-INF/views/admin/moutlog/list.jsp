...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=9508"></script>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/moutlog/queryList",
		data : {
			"page":page,
			"mno":mno,
			"mname":mname,
		...
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				...
				html+='<td>';
				html+='<a href="javascript:void(0)" onclick="del(this)" data-id="'+rows[i].moutlog.id+'" class="btn btn-danger btnStyle">删除</a>';
				html+='</td>';
				html+='</tr>';
			}
			if (rows.length == 0) {
...
function ajaxCount() {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/moutlog/queryCount",
		async: false,
		data : {
			"mno":mno,
			"mname":mname,
...
function del(e){
	if(window.confirm('你确定要删除该物料出库吗')){
		...
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/admin/moutlog/del",
			data : {
				"id":id
			},
			success : function(result) {
