			...
			<div class="ibox float-e-margins">
				...
				<div class="ibox-content">
					...
					<div class="row"  style="margin-top:5px;padding-left: 15px;">
						<a href="${pageContext.request.contextPath}/admin/provider/add" class="btn btn-primary">新增</a>
					</div>
					...
					<div class="table-responsive">
						<table class="table  table-striped table-bordered  table-hover">
							...
							<tbody  id="provider_body">
							</tbody>
						</table>
						<div id="provider_bar" class="pagination" style="margin-top:20px;"></div>
					</div>
				</div>
			</div>
		</div>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=7588"></script>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/provider/queryList",
		data : {
			"page":page,
			"loginName":loginName,
			"gysdj":gysdj
		...
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				html += '<tr>' ;
				var loginNameVal=setNullToEmpty(rows[i].provider.loginName);
				html+='<td>'+ loginNameVal+'</td>';
				var passwordVal=setNullToEmpty(rows[i].provider.password);
				html+='<td>'+ passwordVal+'</td>';
				var celPhoneVal=setNullToEmpty(rows[i].provider.celPhone);
				html+='<td>'+ celPhoneVal+'</td>';
				var emailVal=setNullToEmpty(rows[i].provider.email);
				html+='<td>'+ emailVal+'</td>';
				var addressVal=setNullToEmpty(rows[i].provider.address);
				html+='<td>'+ addressVal+'</td>';
				var remarkVal=setNullToEmpty(rows[i].provider.remark);
				html+='<td>'+ remarkVal+'</td>';
				var createTimeVal=setNullToEmpty(rows[i].provider.createTime);
				html+='<td>'+ createTimeVal+'</td>';
				var gysdjVal=setNullToEmpty(rows[i].gysdjStr);
				html+='<td>'+gysdjVal+'</td>';
				html+='<td>';
				html+='<a href="${pageContext.request.contextPath}/admin/provider/update?id='+rows[i].provider.id+'" class="btn btn-success btnStyle">修改</a>';
				html+='<a href="javascript:void(0)" onclick="del(this)" data-id="'+rows[i].provider.id+'" class="btn btn-danger btnStyle">删除</a>';
				html+='</td>';
				html+='</tr>';
			}
			if (rows.length == 0) {
...
function ajaxCount() {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/provider/queryCount",
		async: false,
		data : {
			"loginName":loginName,
			"gysdj":gysdj
...
function del(e){
	if(window.confirm('你确定要删除该供应商吗')){
		var id = $(e).attr("data-id");
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/admin/provider/del",
			data : {
				"id":id
			},
			success : function(result) {
