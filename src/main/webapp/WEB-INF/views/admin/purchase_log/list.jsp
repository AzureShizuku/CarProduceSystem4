...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=6533"></script>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/purchase_log/queryList",
		data : {
			"page":page,
			"no":no,
			"name":name,
		...
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				...
				html+='<td>';
				html+='<a href="javascript:void(0)" onclick="del1(this)" data-id="'+rows[i].purchaseLog.id+'" class="btn btn-danger btnStyle">删除</a>';
				html+='</td>';
				html+='</tr>';
			}
			if (rows.length == 0) {
...
function ajaxCount() {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/purchase_log/queryCount",
		async: false,
		data : {
			"no":no,
			"name":name,
...
function del1(e){
	if(window.confirm('你确定要删除该订货单吗')){
		...
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/admin/purchase_log/del1",
			data : {
				"id":id
			},
			success : function(result) {
