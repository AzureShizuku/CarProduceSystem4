...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=3480"></script>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/pro_plan/queryList",
		data : {
			"page":page,
			"pname":pname
		},
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				...
				html+='<td>';
				html+='<a href="javascript:void(0)" onclick="del(this)" data-id="'+rows[i].proPlan.id+'" class="btn btn-danger btnStyle">删除</a>';
				html+='</td>';
				html+='</tr>';
			}
			if (rows.length == 0) {
...
function ajaxCount() {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/pro_plan/queryCount",
		async: false,
		data : {
			"pname":pname
		},
...
function del(e){
	if(window.confirm('你确定要删除该生产计划吗')){
		...
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/admin/pro_plan/del",
			data : {
				"id":id
			},
			success : function(result) {
