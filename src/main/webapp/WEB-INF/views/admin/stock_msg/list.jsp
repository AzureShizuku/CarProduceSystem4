			...
			<div class="ibox float-e-margins">
				...
				<div class="ibox-content">
					...
					<div class="row"  style="margin-top:5px;padding-left: 15px;">
						<a href="${pageContext.request.contextPath}/admin/stock_msg/add" class="btn btn-primary">新增</a>
					</div>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=9478"></script>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/stock_msg/queryList",
		data : {
			"page":page,
			"proNo":proNo,
			"proName":proName
		...
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				...
				html+='<td>'+ numVal+'</td>';
				html+='<td>';
				html+='<a href="${pageContext.request.contextPath}/admin/stock_msg/update?id='+rows[i].stockMsg.id+'" class="btn btn-success btnStyle">修改</a>';
				html+='<a href="javascript:void(0)" onclick="del(this)" data-id="'+rows[i].stockMsg.id+'" class="btn btn-danger btnStyle">删除</a>';
				html+='</td>';
				html+='</tr>';
			}
			if (rows.length == 0) {
...
function ajaxCount() {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/stock_msg/queryCount",
		async: false,
		data : {
			"proNo":proNo,
			"proName":proName
...
function del(e){
	if(window.confirm('你确定要删除该库存台账吗')){
		...
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/admin/stock_msg/del",
			data : {
				"id":id
			},
			success : function(result) {
