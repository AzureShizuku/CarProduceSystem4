...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=6574"></script>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/mpurchase_plan/queryList",
		data : {
			"page":page,
			"title":title
		},
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				...
				html+='<td>';
				html+='<a href="javascript:void(0)" onclick="del(this)" data-id="'+rows[i].mpurchasePlan.id+'" class="btn btn-danger btnStyle">删除</a>';
				html+='</td>';
				html+='</tr>';
			}
			if (rows.length == 0) {
...
function ajaxCount() {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/mpurchase_plan/queryCount",
		async: false,
		data : {
			"title":title
		},
...
function del(e){
	if(window.confirm('你确定要删除该物料采购计划吗')){
		var id = $(e).attr("data-id");
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/admin/mpurchase_plan/del",
			data : {
				"id":id
			},
			success : function(result) {
