					...
					<div class="row"   style="width:80%;margin-left:10%">
						<div class="col-lg-6">
							<div class="form-group" id="staffNoParentContent">
								<label>员工号<span style="color:red">(*必填)</span></label>
								<input type="text" value="${data.staffNo}" placeholder="请输入员工号" id="staffNo" class="form-control">
							</div>
							...
							<div class="form-group" id="staffTypeParentContent">
								...
								<select id="staffType"  class="form-control">
									<c:forEach items="${staffTypeList}" var="item">
										<c:if test="${item.id==data.staffType}">
...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=9563"></script>
...
function submitData(){
	...
	$.ajax({
		type: 'post',
		url: '${pageContext.request.contextPath}/admin/staff_msg/update_submit',
		data:{
			"id":'${data.id}',
			"staffNo":staffNo,
			"password":password,
		...
		success: function(result) {
			if(result.code == 0){
				...
				alert(result.msg);
				window.location.href="${pageContext.request.contextPath}/admin/staff_msg";
			}
		}
	});
}
