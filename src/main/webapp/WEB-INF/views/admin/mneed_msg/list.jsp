...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=0612"></script>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/mneed_msg/queryList",
		data : {
			"page":page,
			"title":title,
		...
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				...
				html+='<td>';
				html+='<a href="javascript:void(0)" onclick="del(this)" data-id="'+rows[i].mneedMsg.id+'" class="btn btn-danger btnStyle">删除</a>';
				html+='</td>';
				html+='</tr>';
			}
			if (rows.length == 0) {
...
function ajaxCount() {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/mneed_msg/queryCount",
		async: false,
		data : {
			"title":title,
			"createDate1":createDate1,
...
function del(e){
	if(window.confirm('你确定要删除该物料需求单吗')){
		...
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/admin/mneed_msg/del",
			data : {
				"id":id
			},
			success : function(result) {
